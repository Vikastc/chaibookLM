...
export async function streamWorkspaceChat(
  ...
  const chatModel =
    CHAT_MODELS.find((model: string) => model === requestedModel) ?? CHAT_MODEL;
  ...
  const citations = retrievedChunks.map(
    (chunk: (typeof retrievedChunks)[number]) => ({
  ...
  const systemPrompt = buildChatSystemPrompt({
    ...
    userMemories: userMemories.map(
      (memory: { memory: string }) => memory.memory,
    ),
    webSearchEnabled,
  });
